...
const { AOM_SERVICE_NAME } = process.env;
const {
...
export class HttpExceptionFilter implements ExceptionFilter {
    ...
    // const request = ctx.getRequest();
    if (exception instanceof AomHttpException) {
      const status = exception.getStatus();
      const aomHttpException = exception.getResponse() as ExceptionDTO;

      return response.status(status).json(
        ErrorDTO.plainToClass({
          serviceName: AOM_SERVICE_NAME,
          traceId,
          ...aomHttpException,
        }),
      );
